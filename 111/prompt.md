...
- CDN 依赖：Tailwind CSS（`https://cdn.tailwindcss.com`）+ Google Fonts（Inter 400/500/600/700/800 + Noto Sans SC 400/500/700 + Noto Serif SC 700/900 + Fraunces 400/700 Italic + Caveat 500/700）
- 数据持久化：IndexedDB（优先，清缓存不丢）+ localStorage（兼容回退）
- 支持 Chrome / Edge / Safari / Firefox
- 主题系统支持独立 CSS 文件扩展（如 `theme-editorial.css`）
...
├── CSS/
│   ├── content-os.css         # 全部样式
│   └── theme-editorial.css    # 蓝黄杂志风（独立主题）
├── JS/
│   └── content-os.js          # 全部逻辑
├── IMG/
...
| 冷灰 | cool | 灰白底 + 蓝灰强调 |
| 自然绿 | green | 浅绿底 + 森林绿强调 |
| 高级时尚杂志风 | magazine | 暖白底 + 深棕强调 |
| 草绿小清新风 | fresh | 浅绿底 + 翠绿强调 |
| 暖金 | warmgold | 奶油底 + 金色强调 |
| 蓝黄杂志风 | editorial | 奶油白底 + 蓝黄红（独立文件 `theme-editorial.css`） |
...
### 存储 Key 清单（IndexedDB 为主，localStorage 为兼容回退）
| Key | 内容 |
|-----|------|
...
6. **数据持久化**：优先使用 IndexedDB（清缓存不丢），localStorage 作为兼容回退，首次加载时自动迁移旧数据
7. **新手指南**：5 步引导式弹窗，支持键盘 ←→↑↓/Esc 导航，右上角悬浮胶囊按钮触发
8. **确认弹窗**：不使用浏览器原生 `confirm()`，自定义品牌弹窗 `showConfirm(msg, callback)`
9. **事件泄漏**：document 级 mousedown/mouseup 在 init() 中只绑定一次，render() 不重复添加
10. **导入校验**：校验 topics 数组、必填字段、status 白名单、priority 范围、上限 200 条
